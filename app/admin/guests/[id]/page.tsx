...
export default async function GuestDetailPage({
  ...
  const bookings = (guest as any).bookings ?? []

  const totalNights = bookings.reduce(
    (acc: number, b: any) => acc + (b.totalNights ?? 0), 0
  )
      ...
      <div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-3">
            <StatPill label="Total Stays"   value={bookings.length} />
...
function StatPill({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-lg border border-[--border-color]
      ...
      <p className="font-display text-[24px] font-light text-[--text-color]">
        {value}
      </p>
